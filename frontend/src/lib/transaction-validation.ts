...
export function validateTokenCurrency(
  ...
  try {
    ...
    const isValidToken = validTokens.some(
      (token) => token.encodedCurrency === currency && token.issuer === issuer
    );
    ...
    const validTokenConfig = validTokens.find(
      (token) => token.encodedCurrency === currency && token.issuer === issuer
    );
...
export function extractAmountInfo(
  tx: any,
  logger: ApiLogger
): {
  currency: string;
  ...
  if (
    ...
    txJson.DeliverMax &&
    typeof txJson.DeliverMax === 'object' &&
    txJson.DeliverMax.currency &&
    txJson.DeliverMax.issuer &&
    txJson.DeliverMax.value
  ) {
    amountInfo = {
    ...
    tx.meta.delivered_amount.currency &&
...
export async function validateTransactionIntegrity(
  ...
  try {
    ...
    const tokenValidation = validateTokenCurrency(
      brlusdConfig.currency,
      brlusdConfig.issuer,
      logger
    );
    ...
    const mintTransactions = issuerTransactions.filter((tx: any) => {
      ...
      if (typeof mintAmount === 'object' && mintAmount.currency && mintAmount.issuer) {
        isCorrectCurrency =
          mintAmount.currency === brlusdConfig.currency &&
