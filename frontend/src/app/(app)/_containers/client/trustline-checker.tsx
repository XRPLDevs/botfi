...
import type { TrustlineStatus, TrustlineResponse, TrustlineStatusWithBalance } from '@/app/(app)/_containers/asset-table/types';
...
export function TrustlineChecker({ children }: TrustlineCheckerProps) {
  ...
  const trustlineStatus: TrustlineStatus | null = data ? (() => {
    ...
    primaryTokens.forEach(({ currency }) => {
      const trustline = data.find((t: TrustlineResponse) => 
  ...
  return <>{children(trustlineStatus, trustlineStatusWithBalance, isLoading, error?.message || null)}</>;
}
