...
export async function GET(request: Request, _context: { params: Promise<Record<string, never>> }) {
  ...
  try {
    ...
    logger.debug('Deposit transactions filtered (successful only)', { count: depositTransactions.length });
    ...
    logger.debug('Invalid transactions', { count: invalidTransactions.length, reasons: invalidTransactions.map(t => t.reason) });
    ...
    let mintTransactions: any[] = [];
      ...
      logger.debug('bRLUSD Issuer transactions fetched', { count: issuerTransactions.length, issuer: ISSUERS.bRLUSD });
    ...
    const claimableAmount = claimableDeposits.reduce((sum, deposit) => sum + parseFloat(deposit.amount), 0);
    const totalDeposited = validDeposits.reduce((sum, deposit) => sum + parseFloat(deposit.amount), 0);
    const totalClaimed = claimedDeposits.reduce((sum, deposit) => sum + parseFloat(deposit.amount), 0);
